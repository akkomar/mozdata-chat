...
"""
Submit script for the Mozilla data-docs ingestion pipeline.

This script compiles and submits the data ingestion pipeline to Vertex AI Pipelines.
It supports both immediate execution and scheduled runs.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Mozilla data-docs ingestion pipeline configuration"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="GCS bucket for storing HTML documents",
    )
    parser.add_argument(
        "--gcs-prefix",
        default=os.getenv("GCS_PREFIX", "data-docs"),
        help="GCS prefix for HTML documents",
    )
    parser.add_argument(
        "--docs-base-url",
        default=os.getenv("DOCS_BASE_URL", "https://docs.telemetry.mozilla.org"),
        help="Base URL for citation links (live docs site)",
    )
    parser.add_argument(
        "--service-account",
    ...
    required_params = {
        ...
        "gcs_bucket": parsed_args.gcs_bucket,
    }
...
if __name__ == "__main__":
    ...
    if args.cron_schedule and args.schedule_only:
        ...
        pipeline_job_schedule = aiplatform.PipelineJobSchedule(
            pipeline_job=job,
            display_name=f"{args.pipeline_name} Scheduled Ingestion Job",
        )

        schedule_list = pipeline_job_schedule.list(
            filter=f'display_name="{args.pipeline_name} Scheduled Ingestion Job"',
            project=args.project_id,
            location=args.region,
        )
